...
get_element_test()->
    AssumedResult = 7,
    ActualResult = matrix_as_list_of_lists:get_value(2,3,[[2,3,11],[4,5,12],[6,7,12],[13,14,15]]),
    ?assertEqual(AssumedResult, ActualResult).

set_element_test()->
    AssumedResult = [[2,3,11],[4,5,12],[6,99,12],[13,14,15]],
    ActualResult = matrix_as_list_of_lists:set_value(2,3,99,[[2,3,11],[4,5,12],[6,7,12],[13,14,15]]),
    ?assertEqual(AssumedResult, ActualResult).

set_front_element_test()->
    AssumedResult = [[99,3,11],[4,5,12],[6,7,12],[13,14,15]],
    ActualResult = matrix_as_list_of_lists:set_value(1,1,99,[[2,3,11],[4,5,12],[6,7,12],[13,14,15]]),
    ?assertEqual(AssumedResult, ActualResult).

set_back_element_test()->
    AssumedResult = [[2,3,11],[4,5,12],[6,7,12],[13,14,99]],
    ActualResult = matrix_as_list_of_lists:set_value(3,4,99,[[2,3,11],[4,5,12],[6,7,12],[13,14,15]]),
    ?assertEqual(AssumedResult, ActualResult).

set_back_mid_element_test()->
    AssumedResult = [[2,3,11],[4,5,12],[6,7,99],[13,14,15]],
    ActualResult = matrix_as_list_of_lists:set_value(3,3,99,[[2,3,11],[4,5,12],[6,7,12],[13,14,15]]),
    ?assertEqual(AssumedResult, ActualResult).

full_verification_test()->
    StartMatrix = [[2,3,11],[4,5,12],[6,7,12],[13,14,15]],
    Rows = matrix_as_list_of_lists:rows_sums(StartMatrix),
    ?assertEqual([16,21,25,42], Rows),
    Cols = matrix_as_list_of_lists:cols_sums(StartMatrix),
    ?assertEqual([25,29,50], Cols),
    NewMatrix = matrix_as_list_of_lists:set_value(2,3,50,StartMatrix),
    AssumedResult = [[2,3,11],[4,5,12],[6,50,12],[13,14,15]],
    ?assertEqual(AssumedResult, NewMatrix),
    NewRows = matrix_as_list_of_lists:rows_sums(NewMatrix),
    ?assertEqual([16,21,68,42], NewRows),
    NewCols = matrix_as_list_of_lists:cols_sums(NewMatrix),
